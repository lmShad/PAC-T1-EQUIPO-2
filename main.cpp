...
std::vector<std::string> pedirSospechosos() {
    ...
    // Nota: Ya no hacemos cin.ignore() aquí, lo hacemos en el main para tener control.
...
int main() {
    ...
    // Limpiamos el 'enter' que quedó pendiente después de escribir el nombre del archivo
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
